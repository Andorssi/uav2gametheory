...
"""
add_client: UEを追加する
true_remaining_clients: 実際に残っているUE集合
"""
class Area:
...
class UAV:
    ...
    def own_optimize(self, candidate_area, game):
        C = len(game.field.areas[candidate_area].true_remaining_clients(game.remaining_true))
        L = game.field.field_distance(self.area, candidate_area)
        S = 1 if candidate_area in game.server.observable_area else 0
        return C - alpha * L + gamma * S

    def choose_selfish_area(self, game):
        candidates = game.field.get_k_rinsetu(self.area, move_k)
        return max(candidates, key=lambda area: self.own_optimize(area, game))

    def choose_return_area(self, game):
        candidates = game.field.get_k_rinsetu(self.area, move_k)
        return min(candidates, key=lambda area: game.field.field_distance(area, game.server.area))


"""
is_observable: 領域areaがサーバ観測範囲内か判定
clients_in_area: サーバが認識する領域areaの残存UE集合
...
class Server:
    ...
    def social_score(self, assignment, game):
        ...
        for uav, area in assignment.items():
            covered |= self.clients_in_area(game.field.areas[area])
            total_distance += game.field.field_distance(uav.area, area)
            area_count[area] += 1
...
class Game:
    def __init__(self):
        ...
        observable_area = set(self.field.get_k_rinsetu(B, RANGE_OF_OBSERVE))
    ...
    def collect_clients(self, placement):
        ...
        for uav in sorted(placement.keys(), key=lambda x: x.id):
            ...
            candidates = sorted(
                self.field.areas[area_id].true_remaining_clients(self.remaining_true)
            )
    ...
    def step(self, t):
        ...
        self.print_step(
            t=t,
            placement=placement,
            inside_uavs=inside_uavs,
            outside_uavs=outside_uavs,
